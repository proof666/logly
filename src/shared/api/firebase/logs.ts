...
export function useLogs(
  ...
  useEffect(() => {
    ...
    if (options?.since) {
      constraints.push(
        where("actionDate", ">=", Timestamp.fromMillis(options.since))
      );
    }
    ...
    const unsub = onSnapshot(q, (snap) => {
      const res: LogRecord[] = snap.docs.map((d) => ({
        ...
        itemId,
        actionDate:
          (d.get("actionDate") as Timestamp)?.toMillis?.() ?? Date.now(),
        comment: d.get("comment") ?? undefined,
        createdAt:
          (d.get("createdAt") as Timestamp)?.toMillis?.() ?? Date.now(),
        updatedAt:
          (d.get("updatedAt") as Timestamp)?.toMillis?.() ?? Date.now(),
      }));
      setLogs(res);
      setLoading(false);
    });
  ...
  const updateLog = useCallback(
    async (
      id: LogId,
      patch: Partial<Pick<LogRecord, "actionDate" | "comment">>
    ) => {
      if (!userId || !itemId) throw new Error("Not ready");
      const toUpdate: any = { ...patch, updatedAt: serverTimestamp() };
