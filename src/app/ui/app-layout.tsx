...
export function AppLayout() {
  ...
  const matches = useMatches() as Array<
    { handle?: { title?: string }; params?: Record<string, string> } & any
  >;
  const { user } = useAuth();
  ...
  const itemIdParam = useMemo(() => {
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      ...
      if (m?.params?.itemId) return m.params.itemId as string;
    }
    return undefined;
  }, [matches]);
  ...
  const pageTitle = itemTitle ?? current?.handle?.title ?? "Habit Logger";
  return (
